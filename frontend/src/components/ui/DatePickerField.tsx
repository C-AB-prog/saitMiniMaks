    ...
    <div className={className} ref={rootRef}>
      ...
      >
        <span className="date-picker-trigger-copy">
          <IconCalendar size={14} style={{ color: 'var(--text-tertiary)', flexShrink: 0 }} />
          <strong>{value ? formatDateLabel(value) : 'Выбрать дату'}</strong>
        </span>
        <span className="date-picker-trigger-chevron">
          <IconChevronL size={12} style={{ transform: open ? 'rotate(-90deg)' : 'rotate(-90deg) scaleX(-1)', transition: 'transform 0.15s' }} />
        </span>
      </button>
            ...
            {matrix.map((cell, index) => {
              ...
              return (
                <button
                  ...
                  onClick={() => { onChange(cell.value); setOpen(false); }}
                >
                  {cell.day}
                </button>
              );
