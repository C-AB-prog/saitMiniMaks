...
export const TaskList = ({ tasks, selectedDate, statusFilter, onToggle, onEdit, onDelete }: TaskListProps) => {
  ...
  const showCompletedSection = statusFilter !== 'incomplete' && (selectedDate !== null || statusFilter === 'completed' || completedTasks.length > 0);
      ...
      <div className="task-card-top task-card-top-closer">
        ...
        <div className="task-copy">
          <div className="task-title-row">
            <h3>{task.title}</h3>
            <span className={`task-state-pill ${task.completed ? 'done' : 'active'}`}>{task.completed ? 'Done' : 'In progress'}</span>
          </div>
    ...
    <div className="task-columns task-columns-polished">
      {statusFilter !== 'completed' ? (
        <section className="task-section-card">
          ...
          <div className="stack-md">{activeTasks.length ? activeTasks.map(renderTaskCard) : <EmptyState title="Здесь пусто" description="По текущему фильтру нет активных задач." />}</div>
        </section>
      ) : null}
          ...
          <div className="section-row">
            <div>
              ...
              <p className="muted-text">
                {selectedDate ? 'Отдельный блок completed за конкретную дату.' : 'Теперь completed видны всем участникам Focus, даже без выбора даты.'}
              </p>
            </div>
            <span className="count-pill">{completedTasks.length}</span>
          </div>
          <div className="stack-md">
            {completedTasks.length ? completedTasks.map(renderTaskCard) : <EmptyState title="Пока пусто" description="По текущему фильтру нет выполненных задач." />}
          </div>
        </section>
      ) : null}
    </div>
