...
import { IconX, IconUsers, IconPlus } from '../ui/Icons';
...
export const FocusFormModal = ({ open, mode, initialFocus, onClose, onSubmit }: FocusFormModalProps) => {
  ...
  const existingPhones = useMemo(() => initialFocus?.members?.map((m) => m.phone) ?? [], [initialFocus]);
  const [previewUrl, setPreviewUrl] = useState<string | null>(
    initialFocus?.coverImage ? fileUrl(initialFocus.coverImage) : null
  );
  ...
  const addPhone = () => {
    ...
    if (!normalized) { setPhoneError('Введите номер телефона участника.'); return; }
    if (!isValidPhone(normalized)) { setPhoneError('Номер телефона выглядит некорректно.'); return; }
    if (user && normalized === user.phone) { setPhoneError('Вы уже владелец этого Focus, себя добавлять не нужно.'); return; }
    if (collaboratorPhones.includes(normalized)) { setPhoneError('Этот человек уже добавлен в список участников.'); return; }
    if (collaboratorPhones.length >= 12) { setPhoneError('В одном Focus пока можно добавить до 12 участников.'); return; }
    setCollaboratorPhones((curr) => [...curr, normalized]);
    setPhoneInput('');
    setPhoneError(null);
  };

  const removePhone = (phone: string) => {
    setCollaboratorPhones((curr) => curr.filter((p) => p !== phone));
    setPhoneError(null);
  };
  ...
  return (
    <Modal
      ...
      subtitle="Добавьте обложку, описание и участников по номеру телефона"
      size="wide"
      footer={
        <>
      ...
      <form id="focus-form" className="stack-xl" onSubmit={submit}>
        <div className="modal-intro-card">
          <div>
            ...
            <p>Название, цель, обложка и состав команды в одном потоке.</p>
          </div>
          <div className="inline-chip-row">
            <span className="focus-chip emphasis">
              <IconUsers size={11} />
              {collaboratorPhones.length + 1} участников
            </span>
        ...
        <div className="modal-section-grid">
          <label className={`field-group modal-main-column ${errors.title ? 'is-invalid' : ''}`}>
            ...
            {errors.title ? (
              ...
              <span className="field-hint">Лучше коротко и по сути: продукт, продажи, контент, клиенты.</span>
            )}
          </label>
          ...
          <label className={`field-group modal-main-column ${errors.description ? 'is-invalid' : ''}`}>
            ...
            {errors.description ? (
              ...
              <span className="field-hint">Опишите, ради чего этот Focus существует.</span>
            )}
          </label>
        </div>
        ...
        <div className="focus-settings-grid focus-settings-grid-polished">
          <label className="field-group">
            ...
            <div className="upload-box upload-box-large upload-box-premium">
              ...
              <span className="field-hint">Изображение до 5 MB. Лучше горизонтальная картинка.</span>
            </div>
          </label>
            ...
            <div className="team-box team-box-polished team-box-elevated">
              <div className="team-box-head">
                <div>
                  ...
                  <p>Только зарегистрированные пользователи. Телефон нормализуется автоматически.</p>
                </div>
              ...
              <div className="phone-input-row phone-input-row-polished">
                <input
                  ...
                  onBlur={() => setPhoneInput((curr) => normalizePhone(curr))}
                  onKeyDown={(event) => {
                    if (event.key === 'Enter') { event.preventDefault(); addPhone(); }
                  }}
                  placeholder="+7 930 750 75 43"
                />
                <button className="button button-secondary" type="button" onClick={addPhone} style={{ gap: '6px' }}>
                  <IconPlus size={14} />
                  Добавить
                </button>
              </div>
              ...
              {phoneError ? (
                ...
                <span className="field-hint">Нельзя добавить себя, дубли и больше 12 участников.</span>
              )}
              ...
              <div className="team-chip-grid team-chip-grid-polished">
                {collaboratorPhones.length ? (
                  ...
                  <div className="subtle-empty-card">
                    Пока в Focus только вы. Добавьте людей по номеру телефона.
                  </div>
                )}
              </div>
            </div>
