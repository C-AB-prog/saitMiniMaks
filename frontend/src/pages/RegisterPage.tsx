...
import { IconUser, IconPhone, IconLock } from '../components/ui/Icons';
    ...
    <div className="auth-page auth-page-rich">
      <div className="auth-layout-shell">
        <section className="auth-side-card">
          ...
          <p>
            Регистрация без SMS: имя, телефон и пароль. Дальше вы сразу попадаете в продукт и создаёте первый Focus.
          </p>
          ...
          <div className="auth-side-stats">
            <article>
              ...
              <span>Тёмная premium-подача без системной грубости</span>
            </article>
            <article>
        ...
        <form className="auth-card auth-card-rich" onSubmit={submit}>
          ...
          <label className={`field-group ${errors.name ? 'is-invalid' : ''}`}>
            <span className="field-label">
              <IconUser size={12} style={{ display: 'inline', marginRight: '5px', verticalAlign: 'middle' }} />
              Имя
            </span>
            <input
            ...
            {errors.name ? (
              ...
              <span className="field-hint">Имя будет видно участникам ваших Focus.</span>
            )}
          </label>
          ...
          <label className={`field-group ${errors.phone ? 'is-invalid' : ''}`}>
            ...
            {errors.phone ? (
              ...
              <span className="field-hint">Телефон используется как логин и для приглашений в команду.</span>
            )}
          </label>
          ...
          <label className={`field-group ${errors.password ? 'is-invalid' : ''}`}>
            ...
            <input
              ...
              {...register('password', {
                ...
                validate: (value) => {
                  if (!/[a-z]/.test(value)) return 'Нужна хотя бы одна строчная буква.';
                  if (!/[A-Z]/.test(value)) return 'Нужна хотя бы одна заглавная буква.';
                  if (!/[0-9]/.test(value)) return 'Нужна хотя бы одна цифра.';
                  if (!/[^a-zA-Z0-9]/.test(value)) return 'Нужен хотя бы один спецсимвол (!@#$%...).';
                  return true;
                }
              })}
            />
            {errors.password ? (
              ...
              <span className="field-hint">Строчные + заглавные + цифры + спецсимволы (!@#$%).</span>
            )}
          </label>
          ...
          <div className="auth-footer-cta">
            ...
            <Link className="auth-footer-link auth-footer-link-prominent" to="/login">
              Войти
            </Link>
          </div>
        </form>
